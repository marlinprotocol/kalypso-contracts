...
const findErrorSelectorInAbi = (contractName: string, allFragments: any[], selector: string): Result => {
  ...
  for (const errorFragment of errorFragments) {
    try {
      ...
      if (ef.selector === selector) {
        console.log(`Contract: ${contractName}, Error: ${ef.name}, Selector: ${ef.selector}`);
        return { found: true, errorName: ef.name, contractName, selector };
      }
    } catch (error) {
      console.error(`Error processing fragment in contract ${contractName}:`, error);
    }
  }
