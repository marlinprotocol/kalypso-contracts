...
async function verify(): Promise<string> {
  ...
  //     address: addresses.implementation.stakingManager,
  //     name: "contracts/ERC1967ProxyFlatten.sol:ERC1967Proxy",
  //   });
  // } catch (error) {
  ...
  try {
    verificationResult = await run("verify:verify", {
      address: addresses.implementation.entityKeyRegistry,
      constructorArguments: [],
    });
    console.log({ verificationResult });
  } catch (error) {
    if (error) {
      console.log("EntityKeyRegistry implementation already verified, continuing...");
    } else {
      console.error("Error verifying EntityKeyRegistry implementation:", error);
    }
  }
