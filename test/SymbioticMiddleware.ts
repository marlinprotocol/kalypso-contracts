...
describe("Middleware Contract Tests", function () {
    ...
    beforeEach(async function () {
        ...
        const attestationVerifierFactory = (await ethers.getContractFactory("MockAttestationVerifier", owner)) as MockAttestationVerifier__factory;
    ...
    describe("Contract Deployment and Initialization", function () {
        it("Should set the correct admin and network ID upon initialization", async function () {
            ...
            expect(
                await middleware.hasRole(await middleware.DEFAULT_ADMIN_ROLE(), await admin.getAddress())
            ).to.be.true;
            expect(
        ...
        it("Should prevent initialization with zero admin address", async function () {
            ...
            await expect(
                upgrades.deployProxy(
                    middlewareFactory,
                    [networkId, await attestationVerifier.getAddress(), ethers.ZeroAddress],
                    {
                        initializer: "initialize",
                    }
                )
        ...
        it("Should prevent initialization with zero attestation verifier address", async function () {
            ...
            await expect(
                upgrades.deployProxy(
                    middlewareFactory,
                    [networkId, ethers.ZeroAddress, await admin.getAddress()],
                    {
                        initializer: "initialize",
                    }
                )
        ...
        it("Should prevent initialization with zero network ID", async function () {
            ...
            await expect(
                upgrades.deployProxy(
                    middlewareFactory,
                    [ethers.ZeroHash, await attestationVerifier.getAddress(), await admin.getAddress()],
                    {
                        initializer: "initialize",
                    }
                )
        ...
        it("Deployer should not have admin role", async function () {
            expect(
                await middleware.hasRole(await middleware.DEFAULT_ADMIN_ROLE(), await defaultAddress.getAddress())
            ).to.be.false;
        });
    });
    ...
    describe("Access Control and Role Management", function () {
        it("Only admin can grant roles", async function () {
            ...
            await expect(
                middleware.connect(otherAccount).grantRole(VAULT_CONFIG_SET_ROLE, await otherAccount.getAddress())
        ...
        it("Admin can grant and revoke roles", async function () {
            ...
            expect(
                await middleware.hasRole(await middleware.DEFAULT_ADMIN_ROLE(), await otherAccount.getAddress())
            ).to.be.true;
        ...
        it("Admin can grant and revoke multiple roles", async function () {
            const VAULT_CONFIG_SET_ROLE = await middleware.VAULT_CONFIG_SET_ROLE();
            const NETWORK_UPDATE_ROLE = await middleware.NETWORK_UPDATE_ROLE();

            await middleware.connect(admin).grantRole(VAULT_CONFIG_SET_ROLE, await otherAccount.getAddress());
            await middleware.connect(admin).grantRole(NETWORK_UPDATE_ROLE, await otherAccount.getAddress());

            expect(
                await middleware.hasRole(VAULT_CONFIG_SET_ROLE, await otherAccount.getAddress())
            ).to.be.true;
            expect(
                await middleware.hasRole(NETWORK_UPDATE_ROLE, await otherAccount.getAddress())
            ).to.be.true;

            await middleware.connect(admin).revokeRole(VAULT_CONFIG_SET_ROLE, await otherAccount.getAddress());
            await middleware.connect(admin).revokeRole(NETWORK_UPDATE_ROLE, await otherAccount.getAddress());
    ...
    describe("Vault Configuration", function () {
        ...
        it("Should allow admin to configure a vault", async function () {
            ...
            await middleware.connect(admin).configureVault(vaultAddress, 1); // SlasherType.INSTANT_SLASH

            const vaultInfo = await middleware.vaultInfo(vaultAddress);
            expect(vaultInfo.slasherType).to.equal(1);
        ...
        it("Should prevent configuring a vault with zero address", async function () {
            await expect(
                middleware.connect(admin).configureVault(ethers.ZeroAddress, 1)
            ).to.be.revertedWith("M:CV-Vault cannot be zero address");
        });

        it("Should prevent configuring a vault with undefined slasher type", async function () {
            await expect(middleware.connect(admin).configureVault(vaultAddress, 0)).to.be.revertedWith(
                "M:CV-Invalid slasher type"
            );

            await expect(middleware.connect(admin).configureVault(vaultAddress, 3)).to.be.reverted;
        });

        it("Should prevent non-admin from configuring a vault", async function () {
            const VAULT_CONFIG_SET_ROLE = await middleware.VAULT_CONFIG_SET_ROLE();
            await expect(
                middleware.connect(otherAccount).configureVault(vaultAddress, 1)
            ).to.be.revertedWithCustomError(middleware, "AccessControlUnauthorizedAccount")
            .withArgs(await otherAccount.getAddress(), VAULT_CONFIG_SET_ROLE);
        });

        it("Should prevent configuring a vault with zero slasher address", async function () {
            const vault = await VaultMock__factory.connect(vaultAddress, owner);
            await vault.setSlasher(ethers.ZeroAddress);
            await expect(
                middleware.connect(admin).configureVault(vaultAddress, 1)
            ).to.be.revertedWith("M:CV-Slasher cannot be zero address");
        });

        it("Should be possible to reconfigure a vault", async function () {
            await middleware.connect(admin).configureVault(vaultAddress, 1); // SlasherType.INSTANT_SLASH
            await middleware.connect(admin).configureVault(vaultAddress, 2); // SlasherType.VETO_SLASH

            const vaultInfo = await middleware.vaultInfo(vaultAddress);
            expect(vaultInfo.slasherType).to.equal(2);
        });
    });

    describe("Network Configuration", function () {
        beforeEach(async function () {
            // Grant NETWORK_UPDATE_ROLE to admin
            const NETWORK_UPDATE_ROLE = await middleware.NETWORK_UPDATE_ROLE();
            await middleware.connect(admin).grantRole(NETWORK_UPDATE_ROLE, await admin.getAddress());
        });
        ...
        it("Should prevent non-admin from updating network ID", async function () {
            const NETWORK_UPDATE_ROLE = await middleware.NETWORK_UPDATE_ROLE();
            await expect(
                middleware.connect(otherAccount).updateNetworkId(ethers.keccak256(ethers.toUtf8Bytes("KalypsoTest2")))
            ).to.be.revertedWithCustomError(middleware, "AccessControlUnauthorizedAccount")
            .withArgs(await otherAccount.getAddress(), NETWORK_UPDATE_ROLE);
        });
    ...
    describe("Instant Slashing Functions", function () {
        ...
        beforeEach(async function () {
            ...
            const attestationData = abiEncoder.encode(
                ...
                [enclaveKeySig, [SigningKey.computePublicKey(enclaveKey.publicKey, false).replace("0x04", "0x"), "0x12312313", "0x12312312312342", "0x32145213", Date.now()]]
            );
